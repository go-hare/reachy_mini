  ...
  const handler = async (cmd: string) => {
    switch (cmd) {
      case 'load_app_settings':
        return { show_welcome_recent_projects: true, show_console_output: true, file_mentions_enabled: true, projects_folder: '', ui_theme: 'auto', chat_send_shortcut: 'mod+enter', has_completed_onboarding: true, code_settings: { theme: 'github', font_size: 14, auto_collapse_sidebar: false } }
...
if (typeof document !== 'undefined') describe('App welcome screen recent projects', () => {
  ...
  it('hides recent projects when setting disabled', async () => {
    ...
    ;(invoke as any).mockImplementation(async (cmd: string) => {
      switch (cmd) {
        case 'load_app_settings':
          return { show_welcome_recent_projects: false, show_console_output: true, file_mentions_enabled: true, projects_folder: '', ui_theme: 'auto', chat_send_shortcut: 'mod+enter', has_completed_onboarding: true, code_settings: { theme: 'github', font_size: 14, auto_collapse_sidebar: false } }
        case 'list_recent_projects':
          return []
        case 'check_ai_agents':
          return { agents: [] }
