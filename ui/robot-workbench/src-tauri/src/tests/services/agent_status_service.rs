...
mod tests {
    ...
    async fn agents_report_versions_without_false_upgrade_flag() {
        let probe = FakeProbe::new()
            .with_command("autohand", true, Ok(Some("0.1.0".to_string())))
            .with_package("autohand-cli", Ok(Some("0.1.0".to_string())))
            .with_installed_package("autohand-cli", Ok(Some("0.1.0".to_string())))
