...
const SettingsPage = ({ onClose }: SettingsPageProps) => {
  ...
  const isSelfHosted = (localStorage.getItem('user_mode') || 'selfhosted') === 'selfhosted';
  ...
  function renderGeneral() {
    return (
      <div className="space-y-10 animate-fade-in">
        ...
        {/* Runtime Mode */}
        <section>
          <h3 className="text-[16px] font-semibold text-claude-text mb-5">运行模式</h3>
          <div className="rounded-xl border border-claude-border/60 bg-claude-input px-4 py-4">
            <div className="text-[14px] font-medium text-claude-text">Self-hosted</div>
            <div className="text-[12px] text-claude-textSecondary/70 mt-1">
              当前桌面端固定使用本地后端。模型、API Key 和 Base URL 请直接在 `Models` 页面配置。
            </div>
          </div>
        </section>
      </div>
