...
import { buildApiUrl, getAttachmentUrl } from '../api';
...
async function openFileInFolder(fileId: string) {
  ...
  try {
    ...
    const res = await fetch(buildApiUrl(`/uploads/${encodeURIComponent(fileId)}/path`));
  ...
  useEffect(() => {
    if (isImage) {
      if (isElectron) {
        ...
        setThumbnailUrl(buildApiUrl(`/uploads/${encodeURIComponent(attachment.id)}/raw`));
      } else {
...
const MessageAttachments: React.FC<MessageAttachmentsProps> = ({ attachments, onOpenDocument }) => {
  ...
  const handleClick = async (att: Attachment) => {
    ...
    if (att.file_type === 'image' || (att.mime_type?.startsWith('image/') ?? false)) {
      if (isElectron) {
        setLightboxUrl(buildApiUrl(`/uploads/${encodeURIComponent(att.id)}/raw`));
      } else {
    ...
    if (onOpenDocument && (textExtensions.includes(ext) || (att.mime_type?.startsWith('text/') ?? false))) {
      try {
        ...
        const fetchUrl = isElectron
          ? buildApiUrl(`/uploads/${encodeURIComponent(att.id)}/raw`)
