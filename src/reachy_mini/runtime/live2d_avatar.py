...
def live2d_system_prompt(capabilities: Live2DCapabilities) -> str:
    ...
    return (
        ...
        "Use only those concrete Live2D tools; do not invent string names.\n"
        "Live2D action calls are model-driven: during normal chat you may choose "
        "a fitting native Live2D motion or expression to support the reply, even "
        "when the user did not explicitly ask for an action. If the user explicitly "
        "asks for an action or expression, you MUST call the matching Live2D tool "
        "before replying.\n"
        "Never use keyword matching as a substitute for reasoning. Choose actions "
        "from the user's intent, your planned response, and the native action list. "
        "If no listed action fits the turn, reply in text only.\n"
        "A natural-language promise such as 'I will wave' or 'I look happy' does "
        "not animate the avatar. To animate, call a concrete Live2D tool.\n"
