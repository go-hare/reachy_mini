...
def import_torch() -> Any:
    try:
        ...
        raise ImportError(
            "POSTER-Var emotion classification requires torch. "
            "Install the yolo-vision/local vision dependencies first."
        ) from exc
    return torch


def resolve_torch_device(torch_module: Any, device: str) -> str:
    ...
    if requested == "mps":
        return (
            "mps"
            if getattr(torch_module.backends, "mps", None)
...
class PosterVarEmotionClassifier:
    """Run an exported POSTER-Var model on YOLO face crops."""
    ...
    def _resolve_device(self, device: str) -> str:
        return resolve_torch_device(self._torch, device)
